...
async def get_all_profile(
    ...
    if current_user.role != "officer":
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only officer can get all profile")
...
async def update_profile_endpoint(
    ...
    profile = db.query(Profile).filter(data.user_id == current_user.id).first()
    if not profile:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Profile not found")

    updated_profile = update_profile(db, profile.user_id, data)
    return ProfileResponse(
